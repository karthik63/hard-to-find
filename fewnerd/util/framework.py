...
class FewShotNERModel(nn.Module):
    ...
    def get_predictions(self, pred, label, queries_to_save, query):
        ...
        for qr in queries_to_save:
            len_row = len(qr)
            truncated = pred_row[prev_len:prev_len+len_row]
    ...
    def train(self,
        ...
        while it + 1 < train_iter:
            for _, (support, query) in enumerate(self.train_data_loader):
                ...
                exp.log_metric('loss', self.item(loss.data))
                exp.log_metric('pred_cnt', tmp_pred_cnt)
                exp.log_metric('correct_cnt', correct)
